...
BEGIN_MESSAGE_MAP(CKSJDemoVCDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON_CLOSE_DEVICE, &CKSJDemoVCDlg::OnBnClickedButtonCloseDevice)
	ON_BN_CLICKED(IDC_BUTTON_REFRESH, &CKSJDemoVCDlg::OnBnClickedButtonRefresh)
END_MESSAGE_MAP()
...
void CKSJDemoVCDlg::UpdateDeviceList(void)
{
	...
	for (int i = 0; i<m_nDeviceNum; i++)
	{
		...
		nRet = KSJ_DeviceGetInformationEx(i, &(m_DeviceInfo[i].DeviceType), &(m_DeviceInfo[i].nSerials), &(m_DeviceInfo[i].wFirmwareVersion), &(m_DeviceInfo[i].wFpgaVersion));
		...
		if (nRet == RET_SUCCESS)
		{
			sprintf_s(szMenuItem, _T("Index(%d)-Type(%s)-Serials(%d)-FwVer(%d.%d)-FpgaVer(%d.%d)-%s"), i, g_szDeviceType[m_DeviceInfo[i].DeviceType], m_DeviceInfo[i].nSerials, btMajVersion, btMinVersion, btFpgaMajVersion, btFpgaMinVersion, _T("Has Opened"));
		}
		else
		{
			sprintf_s(szMenuItem, _T("Index(%d)-%s"), i, _T("Opened By Other Process, You Should Close it First in Other Process"));
		}
...
void CKSJDemoVCDlg::OnBnClickedButtonCloseDevice()
{
	if (m_nDeviceCurSel == -1)    return;

	int nRet = KSJ_DeviceClose( m_nDeviceCurSel );
	ShowErrorInfo(nRet);

	UpdateInterfaceMultiProcess();

	// Then You can Open this Device in Other Process.

	// You Shouldn't call other Functions that Can Open this Device implicity, such as KSJ_DeviceGetInformationEx()

	// You Shouldn't Click Refresh Button or Other Button, Because these may Reopen device implicity.
}


void CKSJDemoVCDlg::OnBnClickedButtonRefresh()
{
	UpdateDeviceList();
}


void CKSJDemoVCDlg::UpdateInterfaceMultiProcess()
{
	if (m_nDeviceCurSel == -1)    return;

	KSJ_DEVICESTATUS   Status;
	KSJ_DeviceGetStatus(m_nDeviceCurSel, &Status);
	TCHAR   szOpenStatus[32] = { '\0' };
	sprintf_s(szOpenStatus, Status == KSJ_DS_CLOSE ? _T("Has Closed In Current Process.") : _T("Has Open In Current Process."));

	((CStatic*)GetDlgItem(IDC_STATIC_OPEN_STATUS))->SetWindowText(szOpenStatus);
