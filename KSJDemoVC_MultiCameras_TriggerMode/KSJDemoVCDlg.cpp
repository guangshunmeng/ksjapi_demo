...
void CKSJDemoVCDlg::UpdateInterface()
{
	...
	((CButton*)GetDlgItem(IDC_CHECK_PREVIEWSTART))->SetCheck(PreviewStatus==PS_START);
}
...
void CKSJDemoVCDlg::OnBnClickedCheckPreviewstart()
{
	...
	for (m_nDeviceCurSel = 0; m_nDeviceCurSel < m_nDeviceNum; m_nDeviceCurSel++)
	{

		HWND   hPreviewWnd = ((CStatic*)GetDlgItem(IDC_STATIC_PREVIEWWND + m_nDeviceCurSel))->m_hWnd;

		int    nPreviewWndWidth = 0;
		int    nPreviewWndHeight = 0;
		RECT   rtPreviewWndClient;

		((CStatic*)GetDlgItem(IDC_STATIC_PREVIEWWND + m_nDeviceCurSel))->GetClientRect(&rtPreviewWndClient);
		...
		nRet = KSJ_TriggerModeSet(m_nDeviceCurSel, (KSJ_TRIGGERMODE)nIndex);
		ShowErrorInfo(nRet);

		BOOL bCheck = ((CButton*)GetDlgItem(IDC_CHECK_SET_CALLBACK))->GetCheck();
		if (bCheck)
		{
			KSJ_PreviewSetCallback(m_nDeviceCurSel, CKSJDemoVCDlg::PREVIEWCALLBACK, this);
		}
		else
		{
...
VOID WINAPI CKSJDemoVCDlg::PREVIEWCALLBACK(unsigned char *pImageData, int nWidth, int nHeight, int nBitCount, void *lpContext)
{
...
{
	DrawCrossLine( 640, 480, 100, pImageData, nWidth, nHeight, nBitCount);
}
...
{
	KSJ_ResetDevice(m_nDeviceCurSel);
}
