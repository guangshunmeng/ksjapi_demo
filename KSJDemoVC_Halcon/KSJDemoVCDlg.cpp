...
void CKSJDemoVCDlg::Capture()
{
	...
	nRet = ConvertKSJToHalcon11(pImageData, nCaptureWidth, nCaptureHeight, nCaptureBitCount, &Halcon_Image);
#else
	Hobject  Halcon_Image;
	nRet = ConvertKSJToHalcon10(pImageData, nCaptureWidth, nCaptureHeight, nCaptureBitCount, &Halcon_Image);
#endif
	
	if (nRet == 0)
	{
		write_image(Halcon_Image, "bmp", 0, "KSJToHalcon.bmp");
	}

	BYTE    *pKSJData = NULL;
#ifdef HALCON11
	nRet = ConvertHalcon11ToKSJ(Halcon_Image, &pKSJData);
#else
	nRet = ConvertHalcon10ToKSJ(Halcon_Image, &pKSJData);
#endif

	if (nRet == 0)
	{
		KSJ_HelperSaveToBmp(pKSJData, nCaptureWidth, nCaptureHeight, nCaptureBitCount, "HalconToKSJ.bmp");
	}
...
void CKSJDemoVCDlg::StartCaptureThread(BOOL bStart)
{
	...
	{
#ifdef UNICODE
		m_hCaptureThreadExitEvent = ::CreateEventW(NULL, TRUE, FALSE, NULL);		// Manual Reset, No signal
#else
		m_hCaptureThreadExitEvent = ::CreateEventA(NULL, TRUE, FALSE, NULL);		// Manual Reset, No signal
#endif // !UNICODE
