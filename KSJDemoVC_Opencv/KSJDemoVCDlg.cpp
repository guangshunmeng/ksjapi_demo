...
void CKSJDemoVCDlg::Capture()
{
	...
	if (nRet == 0)
	{
		KSJ_HelperSaveToBmp(pKSJData, cvImg->cols, cvImg->rows, cvImg->step[1] * 8, "OpencvToKSJ.bmp");
	}
	...
	if (bCheck)
	{
		...
		KSJ_HelperSaveToJpg(pKSJData, nCaptureWidth, nCaptureHeight, nCaptureBitCount, 90, szFileName);
#else
		...
		KSJ_HelperSaveToBmp(pKSJData, nCaptureWidth, nCaptureHeight, nCaptureBitCount, szFileName);
#endif
	}
