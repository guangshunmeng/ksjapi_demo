...
BEGIN_MESSAGE_MAP(CKSJDemoVCDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_CHECK_PREVIEW_FFC, &CKSJDemoVCDlg::OnBnClickedCheckPreviewFfc)
	ON_BN_CLICKED(IDC_CHECK_CAPTURE_FFC, &CKSJDemoVCDlg::OnBnClickedCheckCaptureFfc)
	ON_BN_CLICKED(IDC_BUTTON_MAKE_DARK_FRAME, &CKSJDemoVCDlg::OnBnClickedButtonMakeDarkFrame)
...
void CKSJDemoVCDlg::MessageErrorInfo(int nRet)
{
	TCHAR szErrorInfo[256] = { '\0' };
	KSJ_GetErrorInfo(nRet, szErrorInfo);
	::MessageBox(NULL, szErrorInfo, _T("KSJDemo"), 0);
}
...
void CKSJDemoVCDlg::OnBnClickedButtonCaptureFovSet()
{
	...
	KSJ_SetFlatFieldCorrectionFieldOfView(m_nDeviceCurSel, nColStart, nRowStart, nColSize, nRowSize, ColAddressMode, RowAddressMode, 1);
}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
void CKSJDemoVCDlg::UpdateInterfaceFFC()
{
	CSpinButtonCtrl * pSpinCtrlDarkFramesNum = (CSpinButtonCtrl *)GetDlgItem(IDC_SPIN_DARK_FRAMES_NUM);
	pSpinCtrlDarkFramesNum->SetBuddy(GetDlgItem(IDC_EDIT_DARK_FRAMES_NUM));
	pSpinCtrlDarkFramesNum->SetBase(10);
	pSpinCtrlDarkFramesNum->SetRange32(1, 20);
	pSpinCtrlDarkFramesNum->SetPos32(1);


	CSpinButtonCtrl * pSpinCtrlFlatFieldFramesNum = (CSpinButtonCtrl *)GetDlgItem(IDC_SPIN_FLAT_FIELD_FRAMES_NUM);
	pSpinCtrlFlatFieldFramesNum->SetBuddy(GetDlgItem(IDC_EDIT_FLAT_FIELD_FRAMES_NUM));
	pSpinCtrlFlatFieldFramesNum->SetBase(10);
	pSpinCtrlFlatFieldFramesNum->SetRange32(1, 20);
	pSpinCtrlFlatFieldFramesNum->SetPos32(1);

}
void CKSJDemoVCDlg::OnBnClickedButtonMakeDarkFrame()
{
	if (m_nDeviceCurSel == -1)    return;
	int nDarkFramesNum = GetDlgItemInt(IDC_EDIT_DARK_FRAMES_NUM);

	int nRet = KSJ_MakeDarkFrame(m_nDeviceCurSel, nDarkFramesNum);

	MessageErrorInfo(nRet);
}


void CKSJDemoVCDlg::OnBnClickedButtonMakeFlatFieldFrames()
{
	if (m_nDeviceCurSel == -1)    return;
	int nFlatFieldFramesNum = GetDlgItemInt(IDC_EDIT_FLAT_FIELD_FRAMES_NUM);

	int nRet = KSJ_MakeFlatFieldFrame(m_nDeviceCurSel, nFlatFieldFramesNum);

	MessageErrorInfo(nRet);

}

void CKSJDemoVCDlg::OnBnClickedCheckPreviewFfc()
{
	if (m_nDeviceCurSel == -1)    return;
	BOOL bCheck = ((CButton*)GetDlgItem(IDC_CHECK_PREVIEW_FFC))->GetCheck();

	KSJ_PreviewSetFlatFieldCorrection(m_nDeviceCurSel, bCheck);
}


void CKSJDemoVCDlg::OnBnClickedCheckCaptureFfc()
{
	if (m_nDeviceCurSel == -1)    return;
	BOOL bCheck = ((CButton*)GetDlgItem(IDC_CHECK_CAPTURE_FFC))->GetCheck();

	KSJ_CaptureSetFlatFieldCorrection(m_nDeviceCurSel, bCheck);
}
...
void CKSJDemoVCDlg::OnBnClickedButtonSaveFfc()
{
	CString strFilter = _T("ffc File(*.ffc)|*.ffc|All Files(*.*)|*.*||");

	CFileDialog FileDlg(FALSE, TEXT(""), _T("ksj.ffc"), NULL, strFilter);
	...
	KSJ_SaveFlatFieldCorrection(m_nDeviceCurSel, (LPTSTR)(LPCTSTR)m_strFFCFile);

}


void CKSJDemoVCDlg::OnBnClickedButtonLoadFfc()
{
	CString strFilter = _T("ffc File(*.ffc)|*.ffc|All Files(*.*)|*.*||");

	CFileDialog FileDlg(TRUE, TEXT(""), _T("ksj.ffc"), NULL, strFilter);
