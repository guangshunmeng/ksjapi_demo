...
BEGIN_MESSAGE_MAP(CKSJDemoVCDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_CHECK_16bit, &CKSJDemoVCDlg::OnBnClickedCheck16bit)
END_MESSAGE_MAP()
...
void CKSJDemoVCDlg::Capture()
{
	...
	if (bCheck)
	{
		...
		bCheck = ((CButton*)GetDlgItem(IDC_CHECK_16bit))->GetCheck();
		if (bCheck)
		{
			sprintf_s(szFileName, _T("capture-%04d-%02d-%02d-%02d-%02d-%02d-%03d-%05d.Tiff"), LocalTime.wYear, LocalTime.wMonth, LocalTime.wDay, LocalTime.wHour, LocalTime.wMinute, LocalTime.wSecond, LocalTime.wMilliseconds, (int)fElapse);
			KSJ_HelperSaveToTiff(pImageData, nCaptureWidth, nCaptureHeight, nCaptureBitCount, nCaptureBitCount >> 3, szFileName);
		}
		else
		{
#ifdef SAVE_JPG
...
void CKSJDemoVCDlg::Update16bit()
{
	if (m_nDeviceCurSel == -1)    return;
	...
	if (nSupport)
	{
		GetDlgItem(IDC_CHECK_16bit)->EnableWindow(true);
		bool bEnable;
		KSJ_GetData16Bits(m_nDeviceCurSel, &bEnable);
		((CButton*)GetDlgItem(IDC_CHECK_16bit))->SetCheck(bEnable);
	}
	else
	{
		((CButton*)GetDlgItem(IDC_CHECK_16bit))->SetCheck(false);
		GetDlgItem(IDC_CHECK_16bit)->EnableWindow(false);
	}
}

void CKSJDemoVCDlg::OnBnClickedCheck16bit()
{
	if (m_nDeviceCurSel == -1)    return;
	BOOL bCheck = ((CButton*)GetDlgItem(IDC_CHECK_16bit))->GetCheck();
	if (bCheck)
	{
		KSJ_SetData16Bits(m_nDeviceCurSel, true);
	}
	else
	{
		KSJ_SetData16Bits(m_nDeviceCurSel, false);
	}
}